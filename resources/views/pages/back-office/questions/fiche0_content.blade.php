<div class='row'>
       ...
       @if($question->type_question=="textarea")
        ...
        @if($question->type_question=="checkbox")
                    ...
                    <div class='row'>
                      ...
                      <input type='checkbox' id='{{ $item->libelle_option }}' name='{{$question->id}}[]' value="{{ $item->etiquette_option }}" class='action-chekbox-yes'><span style='padding-top: 7px;'> {{ $item->etiquette_option }}</span>
                      @endif
                    @endforeach
                  </div>
                </div>
  ...
  <hr>
    <a href="{{ route('audits.show',$audit) }}" name='previous' class="previous action-button" >Annuler l'audit</a>
